...
create table if not exists public.coin_transactions (
  ...
  transaction_type text not null check (transaction_type in ('correct_answer', 'streak_bonus', 'review_complete', 'memory_increase', 'first_study')),
  source_type text not null check (source_type in ('quiz', 'review', 'memory', 'study')),
  source_id uuid not null,
  created_at timestamptz not null default now(),
  unique (user_id, transaction_type, source_type, source_id)
);
...
begin
  ...
  on conflict (user_id, transaction_type, source_type, source_id) do nothing
  ...
  if p_transaction_type in ('streak_bonus', 'review_complete', 'memory_increase', 'first_study') then
...
alter table public.coin_transactions
  ...
  check (transaction_type in ('correct_answer', 'streak_bonus', 'review_complete', 'memory_increase', 'first_study', 'room_purchase'));
...
alter table public.coin_transactions
  ...
  check (source_type in ('quiz', 'review', 'memory', 'study', 'room_item'));
...
values
  ('basic_chair', '기본 의자', 'chair', 30, 'assets/room/basic_chair.png', true),
  ('basic_desk', '기본 책상', 'desk', 60, 'assets/room/basic_desk.png', true),
  ('small_plant', '작은 화분', 'plant', 25, 'assets/room/small_plant.png', true),
  ('basic_lamp', '기본 스탠드', 'lamp', 40, 'assets/room/basic_lamp.png', true)
...
alter table public.coin_transactions
  add constraint coin_transactions_transaction_type_check
  check (transaction_type in ('correct_answer', 'streak_bonus', 'review_complete', 'memory_increase', 'first_study', 'room_purchase', 'streak_7_bonus', 'streak_30_bonus', 'streak_100_bonus'));
...
alter table public.coin_transactions
  add constraint coin_transactions_source_type_check
  check (source_type in ('quiz', 'review', 'memory', 'study', 'room_item', 'streak'));
