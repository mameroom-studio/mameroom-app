...
insert into storage.buckets (id, name, public)
values ('pdf_uploads', 'pdf_uploads', false)
on conflict (id) do nothing;

drop policy if exists "Users can upload own PDF analysis files" on storage.objects;
drop policy if exists "Users can read own PDF analysis files" on storage.objects;

create policy "Users can upload own PDF analysis files"
  on storage.objects
  for insert
  to authenticated
  with check (
...
create policy "Users can read own PDF analysis files"
  on storage.objects
  for select
  to authenticated
  using (
